#!/usr/bin/env python3
# OpenAI-compatible version (auto-converted)
...
def _to_oai_tools(anthropic_tools):
...
class _ToolUseBlock:
    def __init__(self, tc):
        ...
        self.input = _json.loads(tc.function.arguments)
...
class _OaiResponseWrapper:
    """Wraps OpenAI response to look like Anthropic response."""
...
def _call_llm(system, messages, tools, max_tokens):
    """Wrapper: convert Anthropic-style call to OpenAI chat completions."""
    oai_msgs = []
    if system:
    ...
    for m in messages:
        ...
        if role == "user":
                ...
                for item in content:
                    if isinstance(item, dict):
                        if item.get("type") == "tool_result":
                            ...
                            oai_msgs.append({"role": "user", "content": item.get("text", "")})
        elif role == "assistant":
            if isinstance(content, list):
                ...
                oai_msgs.append(amsg)
            elif hasattr(content, "content"):
                oai_msgs.append(content)
            elif isinstance(content, str):
                oai_msgs.append({"role": "assistant", "content": content})

    resp = client.chat.completions.create(
        model=MODEL, messages=oai_msgs, tools=oai_tools, max_tokens=max_tokens)
...
def run_subagent(prompt: str, agent_type: str = "Explore") -> str:
    ...
    resp = None
    for _ in range(30):
        resp = _call_llm("", sub_msgs, sub_tools, 8000)
...
def auto_compact(messages: list) -> list:
    ...
    resp = _call_llm("", [{"role": "user", "content": f"Summarize for continuity:\n{conv_text}"}], None, 2000)
    ...
    def _loop(self, name: str, role: str, prompt: str):
        ...
        while True:
            ...
            for _ in range(50):
                ...
                try:
                    response = _call_llm(sys_prompt, messages, tools, 8000)
                except Exception:
                    self._set_status(name, "shutdown")
                    return
...
def agent_loop(messages: list):
    ...
    while True:
        ...
        # LLM call
        response = _call_llm(SYSTEM, messages, TOOLS, 8000)
        messages.append({"role": "assistant", "content": response.content})
        if response.stop_reason != "tool_use":
            return
        # Tool execution
