#!/usr/bin/env python3
# OpenAI-compatible version of s04_subagent.py
"""
s04_subagent.py - Subagents (OpenAI-compatible)
...
def run_subagent(prompt: str) -> str:
    """Fresh context, filtered tools, summary-only return."""
    oai_messages = [
    ...
    for _ in range(30):
        response = client.chat.completions.create(
            model=MODEL, messages=oai_messages, tools=CHILD_TOOLS, max_tokens=8000,
        )
        ...
        for tc in msg.tool_calls:
            ...
            oai_messages.append({"role": "tool", "tool_call_id": tc.id, "content": str(output)[:50000]})
    return msg.content or "(no summary)"
...
def agent_loop(messages: list):
    ...
    while True:
        response = client.chat.completions.create(
            model=MODEL, messages=oai_messages, tools=PARENT_TOOLS, max_tokens=8000,
        )
        msg = response.choices[0].message
